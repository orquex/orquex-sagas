...
public class CheckoutTask implements TaskImplementation {
    ...
    public Map<String, Serializable> execute(String transactionId,
        ...
        if (hasProducts && hasPayload) {
            final var products = mapper.convertValue(metadata.get("products"),
                    new TypeReference<List<Product>>() {
                    });
            final var request = mapper.convertValue(payload, ServiceRequest.class);
            final var product = products.stream().filter(p -> p.size.equals(request.size))
                    .findFirst().orElseThrow(() -> new WorkflowException(
                            String.format("Size '%s' not found", request.size)));
