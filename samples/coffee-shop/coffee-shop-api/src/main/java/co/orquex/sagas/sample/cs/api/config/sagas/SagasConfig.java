package co.orquex.sagas.sample.cs.api.config.sagas;
...
import co.orquex.sagas.sample.cs.api.event.CheckpointListener;
...
public class SagasConfig {
    ...
    public WorkflowExecutor workflowExecutor(
        ...
        return new WorkflowExecutor(stageRequestEventManager, flowRepository,
    ...
    public WorkflowStageExecutor workflowHandler(
            ...
            final FlowRepository flowRepository,
            final TransactionRepository transactionRepository) {
        return new WorkflowStageExecutor(stageRequestEventManager, flowRepository,
                transactionRepository);
    }

    @Bean
    public DefaultStageExecutor defaultStageExecutor(
            final Registry<TaskExecutor> taskExecutorRegistry, final TaskRepository taskRepository,
            final EventManager<Checkpoint> eventManager) {
        // Decorate the strategy implementation with an event handler
        final var activityStrategy = new EventHandlerProcessingStrategy<>(
                new ActivityProcessingStrategy(taskExecutorRegistry, taskRepository), eventManager);
        final var evaluationStrategy = new EventHandlerProcessingStrategy<>(
    ...
    public EventManager<StageRequest> stageRequestEventManager(
            List<StageExecutor> stageExecutorList) {
        final var eventManager = new EventManager<StageRequest>();
        final var stageExecutorRegistry = InMemoryStageExecutorRegistry.of(stageExecutorList);
